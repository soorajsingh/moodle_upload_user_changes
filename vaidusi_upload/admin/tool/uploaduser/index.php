...
$STD_FIELDS = array('id', 'username', 'email',
        ...
        'interests', 'branch', 'dept',
    );
	//chetan_edit
	//department already exists hence dept used 
...
if ($formdata = $mform2->is_cancelled()) {
    ...
    while ($line = $cir->next()) {
        ...
        if ($optype == UU_USER_ADDNEW or $optype == UU_USER_ADDINC) {
			...
			//final upload errors on adding users 
            if(!empty($user->branch) && empty($user->dept)) {
                    $upt->track('status', 'dept not given', 'error');
                    $upt->track('dept', $errorstr, 'error');
                    $error = true;
            }//chetan_edit
            if(empty($user->branch) && !empty($user->dept)) {
                    $upt->track('status', 'branch not given', 'error');
                    $upt->track('branch', $errorstr, 'error');
                    $error = true;
            }//chetan_edit
			if(!empty($user->branch)) {
				if (!$DB->record_exists('loc_framework', array('idnumber'=>$user->branch, 'deleted'=>0))) {
					$upt->track('status', 'branch does not exist', 'error');
					$upt->track('branch', $errorstr, 'error');
					$error = true;
				}
			}//chetan_edit
			if(!empty($user->dept)) {
				if (!$DB->record_exists('loc', array('idnumber'=>$user->dept, 'deleted'=>0))) {
					$upt->track('status', 'department does not exist', 'error');
					$upt->track('dept', $errorstr, 'error');
					$error = true;
				}
			}//chetan_edit
			if(!empty($user->branch) && !empty($user->dept)) {
				if ($DB->record_exists('loc_framework', array('idnumber'=>$user->branch, 'deleted'=>0))) {
					if ($DB->record_exists('loc', array('idnumber'=>$user->dept, 'deleted'=>0))) {
						$branch=$DB->get_record('loc_framework', array('idnumber'=>$user->branch, 'deleted'=>0));
						if(!$DB->record_exists('loc', array('idnumber'=>$user->dept, 'frameworkid'=>$branch->id, 'deleted'=>0))) {
							$upt->track('status', 'department does not exist in branch', 'error');
							$upt->track('branch', $errorstr, 'error');
							$upt->track('dept', $errorstr, 'error');
							$error = true;
						}
        ...
        if ($existinguser) {
			...
			//final upload errors on updating users 
            if(!empty($user->branch) && empty($user->dept)) {
                    $upt->track('status', 'dept not given', 'error');
                    $upt->track('dept', $errorstr, 'error');
                    $userserrors++;
            ...
            if ($updatetype != UU_UPDATE_NOCHANGES and !$remoteuser) {
                ...
                foreach ($allcolumns as $column) {
                    ...
                    if ($existinguser->$column !== $user->$column) {
						...
						//update branch and dept 
						if ($column === 'branch') { //chetan_edit
							//user gives idnumber in csv, but we have to set the actual id in table
							if(!empty($user->$column)) {
								$branch = $DB->get_record('loc_framework', array('idnumber'=>$user->$column, 'deleted'=>0));
								$existinguser->$column = $branch->id;
								$doupdate = true;
								continue;
							}
						}
							
                        if ($column === 'dept') { //chetan_edit
							//user gives idnumber in csv, but we have to set the actual id in table
							if(!empty($user->$column)) {
								$dept = $DB->get_record('loc', array('idnumber'=>$user->$column, 'deleted'=>0));
								$existinguser->$column = $dept->id;
								$doupdate = true;
								continue;
							}
                        }
			...
			//set branch and dept on adding user 
			//chetan_edit
			//user gives idnumber in csv, but we have to set the actual id in table
			if(!empty($user->branch)) {	
				$branch = $DB->get_record('loc_framework', array('idnumber'=>$user->branch, 'deleted'=>0));
			...
			if(!empty($user->dept)) {
				$dept = $DB->get_record('loc', array('idnumber'=>$user->dept, 'deleted'=>0));
				$user->dept = $dept->id;
			}
...
while ($linenum <= $previewrows and $fields = $cir->next()) {
	...
	//preview errors 
    if(!empty($rowcols['branch']) && empty($rowcols['dept'])) {
            $rowcols['status'][] = 'dept not given';
    }//chetan_edit
    if(empty($rowcols['branch']) && !empty($rowcols['dept'])) {
            $rowcols['status'][] = 'branch not given';
    }//chetan_edit
	if(!empty($rowcols['branch'])) {
		if (!$DB->record_exists('loc_framework', array('idnumber'=>$rowcols['branch'], 'deleted'=>0))) {
			$rowcols['status'][] = 'branch does not exist';
		}
	}//chetan_edit
	if(!empty($rowcols['dept'])) {
		if (!$DB->record_exists('loc', array('idnumber'=>$rowcols['dept'], 'deleted'=>0))) {
	...
	if(!empty($rowcols['branch']) && !empty($rowcols['dept'])) {
		if ($DB->record_exists('loc_framework', array('idnumber'=>$rowcols['branch'], 'deleted'=>0))) {
			if ($DB->record_exists('loc', array('idnumber'=>$rowcols['dept'], 'deleted'=>0))) {
				$branch=$DB->get_record('loc_framework', array('idnumber'=>$rowcols['branch'], 'deleted'=>0));
				if(!$DB->record_exists('loc', array('idnumber'=>$rowcols['dept'], 'frameworkid'=>$branch->id, 'deleted'=>0))) {
					$rowcols['status'][] = 'dept does not exist in branch';
				}
			}
		}
	}//chetan_edit
